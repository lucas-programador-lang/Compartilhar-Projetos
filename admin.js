/* =========================================================
   COMPARTILHAR PROJETOS — ADMIN.JS (v16 - Mobile Layout & Typing)
   ========================================================= */
...
(function () {
  ...
  function confirmAction(message, opts) {
    ...
    return new Promise((resolve) => {
      const overlay = qs("#confirmOverlay"); const box = overlay.querySelector(".confirm-box");
      qs("#confirmTitle").textContent = opts.title || "Confirmar ação"; qs("#confirmMessage").textContent = message;
      qs("#confirmOkBtn").textContent = opts.confirmLabel || "Confirmar"; qs("#confirmCancelBtn").textContent = opts.cancelLabel || "Cancelar";
      box.classList.toggle("is-neutral", !!opts.neutral); qs("#confirmIcon").textContent = opts.neutral ? "?" : "!";
      overlay.classList.add("open"); qs("#confirmOkBtn").focus();
      function settle(result) { overlay.classList.remove("open"); qs("#confirmOkBtn").removeEventListener("click", onOk); qs("#confirmCancelBtn").removeEventListener("click", onCancel); resolve(result); }
      function onOk() { settle(true); } function onCancel() { settle(false); }
      qs("#confirmOkBtn").addEventListener("click", onOk); qs("#confirmCancelBtn").addEventListener("click", onCancel);
    });
  }
  ...
  function bindThemeToggle() {
    const themeToggle = qs("#themeToggle") || qs("#themeToggleAdmin"); const themeIcon = qs("#themeIcon");
 ...
 function boot() {
    ...
    // Injetar Correção de CSS para Dispositivos Móveis - Botões Perfeitamente Alinhados e Typings
    if (!document.getElementById("adminChatFixCSS")) {
        ...
        adminChatFix.innerHTML = `
          .admin-chat-header { display: flex; flex-wrap: wrap; gap: 12px; align-items: center; justify-content: space-between; }
          .admin-chat-header > div:last-child, .admin-chat-header .chat-actions { display: flex; gap: 8px; }
          @media (max-width: 768px) {
              .admin-chat-header { flex-direction: column !important; align-items: stretch !important; padding: 16px 20px !important; gap: 16px !important; }
              .admin-chat-header > div:first-child { text-align: center; justify-content: center; width: 100%; }
              .admin-chat-header > div:last-child, .admin-chat-header .chat-actions, .admin-chat-header > button { 
                  display: flex !important; width: 100% !important; gap: 10px !important; justify-content: space-between !important; margin: 0 !important;
              }
              #endActiveChatBtn, #closeActiveChatBtn { flex: 1 !important; margin: 0 !important; padding: 12px 0 !important; font-size: 14.5px !important; height: 44px !important; display: flex; align-items: center; justify-content: center; }
          }
        `;
        document.head.appendChild(adminChatFix);
    }

    if (auth.currentUser) { try { bindSupportChat(); } catch (e) { console.error("Erro ao iniciar chat:", e); } }
    renderAll();
  }
  ...
  function bindSupportChat() {
    ...
    if (input) {
        input.addEventListener("input", () => {
            ...
            notificarDigitacao(activeChatUserId, "admin", true);
            clearTimeout(adminTypingTimeout);
            adminTypingTimeout = setTimeout(() => notificarDigitacao(activeChatUserId, "admin", false), 2000);
        });
    }
  ...
  function renderAdminChatList() {
    ...
    if (sortedChats.length === 0) { listEl.innerHTML = `<p class="muted text-center" style="padding:20px; font-size:14px;">Sem conversas iniciadas.</p>`; if (badge) badge.style.display = "none"; return; }
  ...
  function renderAdminActiveChat() {
    ...
    msgs.forEach(msg => {
        ...
        if (dateLabel !== lastDateLabel) {
            const sep = document.createElement("div"); sep.style.cssText = "text-align: center; font-size: 11px; color: var(--ink-500); margin: 8px 0; font-weight: 700; text-transform: uppercase;"; sep.textContent = dateLabel; msgsEl.appendChild(sep); lastDateLabel = dateLabel;
        }

        const div = document.createElement("div"); div.className = msg.sender === "admin" ? "admin-sent" : "user-received";
    ...
    if (chat.typing_user) {
        ...
        typeDiv.innerHTML = `<div style="font-size: 11px; color: var(--ink-400); font-style: italic; padding: 4px 0;">${escapeHtml(chat.userName || 'Usuário')} está a escrever...</div>`;
        msgsEl.appendChild(typeDiv);
    }
  ...
  function renderOverview() {
    const activeSubs = db.users.filter(isSubActive).length; const pendingWithdrawals = db.withdrawals.filter((w) => w.status === "pending").reduce((s, w) => s + w.amount, 0); const publishedCount = db.projects.filter((p) => p.status === "published").length; const pendingModerationCount = db.projects.filter((p) => p.status === "pending").length; qs("#statGrid").innerHTML = [ stat("Usuários cadastrados", db.users.length), stat("Assinaturas ativas", activeSubs), stat("Projetos publicados", publishedCount), stat("Projetos aguardando moderação", pendingModerationCount, pendingModerationCount > 0), stat("Publicações na comunidade", db.posts.length), stat("Receita estimada", fmtBRL(estimateRevenue()), true), stat("Saques aguardando aprovação", fmtBRL(pendingWithdrawals), true), ].join("");
    const recent = db.projects.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 8); qs("#recentProjectsTable tbody").innerHTML = recent.map((p) => { const owner = userById(p.ownerId); return `<tr><td>${escapeHtml(p.title)}</td><td>${escapeHtml(owner ? owner.name : "—")}</td><td>${escapeHtml(catName(p.categoryId))}</td><td>${fmtDate(p.createdAt)}</td></tr>`; }).join("") || `<tr><td colspan="4" class="muted text-center">Nenhum projeto ainda.</td></tr>`;
  }
  ...
  function renderUsers(filter) {
    filter = (filter || "").toLowerCase(); const list = db.users.filter((u) => u && (!filter || u.name.toLowerCase().includes(filter) || u.email.toLowerCase().includes(filter))); qs("#usersTable tbody").innerHTML = list.map((u) => { const active = isSubActive(u); return `<tr><td>${escapeHtml(u.name)}</td><td>${escapeHtml(u.email)}</td><td>${u.role === "admin" ? `<span class="badge badge-blue">Admin</span>` : `<span class="badge badge-neutral">Usuário</span>`}</td><td>${active ? `<span class="badge badge-success">Ativa</span>` : `<span class="badge badge-neutral">Sem assinatura</span>`}</td><td>${u.suspended ? `<span class="badge badge-danger">Suspenso</span>` : `<span class="badge badge-success">Ok</span>`}</td><td class="flex gap-1">${u.role !== "admin" ? `<button class="btn btn-sm ${u.suspended ? "btn-ghost" : "btn-danger"}" data-suspend="${u.id}">${u.suspended ? "Reativar" : "Suspender"}</button><button class="btn btn-sm btn-ghost" data-promote="${u.id}">Tornar admin</button><button class="btn btn-sm btn-danger" data-deluser="${u.id}">Excluir</button>` : `<button class="btn btn-sm btn-ghost" data-demote="${u.id}">Remover admin</button>`}</td></tr>`; }).join("") || `<tr><td colspan="6" class="muted text-center">Nenhum usuário encontrado.</td></tr>`;
    qsa("[data-suspend]").forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { const u = userById(btn.getAttribute("data-suspend")); const result = await adminFetch("/admin/toggle-suspend", { targetUserId: u.id }); toast(result.suspended ? "Usuário suspenso." : "Usuário reativado.", "success"); renderAll(); }))); qsa("[data-promote]").forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { const u = userById(btn.getAttribute("data-promote")); const ok = await confirmAction(`Tornar "${u.name}" um administrador?`, { title: "Promover a admin", neutral: true, confirmLabel: "Sim, promover" }); if (!ok) return; await adminFetch("/admin/set-role", { targetUserId: u.id, role: "admin" }); toast(`${u.name} agora é administrador.`, "success"); renderAll(); }))); qsa("[data-demote]").forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { const u = userById(btn.getAttribute("data-demote")); if (u.id === currentUser().id) { toast("Você não pode remover seu próprio acesso de administrador.", "error"); return; } const ok = await confirmAction(`Remover o acesso de administrador de "${u.name}"?`, { title: "Remover admin", neutral: true, confirmLabel: "Sim, remover" }); if (!ok) return; await adminFetch("/admin/set-role", { targetUserId: u.id, role: "user" }); toast(`Acesso de administrador removido de ${u.name}.`, "success"); renderAll(); }))); qsa("[data-deluser]").forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { const ok = await confirmAction("Excluir este usuário permanentemente? Isto remove o perfil do banco de dados.", { title: "Excluir usuário" }); if (!ok) return; await adminFetch("/admin/delete-user", { targetUserId: btn.getAttribute("data-deluser") }); toast("Usuário excluído do banco de dados.", "success"); renderAll(); })));
  }

  function renderSubscriptions() {
    const subbed = db.users.filter((u) => u && u.subscription && u.subscription.plan); const active = subbed.filter(isSubActive).length; qs("#subStatGrid").innerHTML = [ stat("Assinaturas ativas", active), stat("Assinaturas expiradas", subbed.length - active), stat("Receita estimada", fmtBRL(estimateRevenue()), true) ].join(""); qs("#subsTable tbody").innerHTML = subbed.map((u) => { const active = isSubActive(u); return `<tr><td>${escapeHtml(u.name)}</td><td>${PLAN_NAMES[u.subscription.plan] || "—"}</td><td>${fmtDate(u.subscription.expiresAt)}</td><td>${ active ? `<span class="badge badge-success">Ativa</span>` : `<span class="badge badge-danger">Expirada</span>` }</td></tr>`; }).join("") || `<tr><td colspan="4" class="muted text-center">Nenhuma assinatura registrada.</td></tr>`;
  }

  function renderProjects(filter) {
    filter = (filter || "").toLowerCase(); const list = db.projects.filter((p) => p && (!filter || p.title.toLowerCase().includes(filter))).sort((a, b) => { if (a.status === "pending" && b.status !== "pending") return -1; if (b.status === "pending" && a.status !== "pending") return 1; return new Date(b.createdAt) - new Date(a.createdAt); });
    qs("#projectsTable tbody").innerHTML = list.map((p) => { const owner = userById(p.ownerId); let statusBadge = p.status === "published" ? '<span class="badge badge-success mt-1">Aprovado</span>' : p.status === "rejected" ? '<span class="badge badge-danger mt-1">Rejeitado</span>' : '<span class="badge badge-warning mt-1">Pendente</span>'; let actionBtns = p.status === "pending" ? `<button class="btn btn-sm btn-primary" style="width: 100%" data-approve-proj="${p.id}">Aprovar</button><button class="btn btn-sm btn-danger" style="width: 100%" data-reject-proj="${p.id}">Reprovar</button>` : `<button class="btn btn-sm btn-danger" style="width: 100%" data-delproj="${p.id}">Excluir</button>`; return `<tr><td><a href="index.html#/projeto/${p.id}" target="_blank" class="link">${escapeHtml(p.title)}</a></td><td>${escapeHtml(owner ? owner.name : "—")}</td><td>${escapeHtml(catName(p.categoryId))}</td><td>${fmtDate(p.createdAt)}<br>${statusBadge}</td><td class="flex gap-1" style="flex-direction: column;">${actionBtns}</td></tr>`; }).join("") || `<tr><td colspan="5" class="muted text-center">Nenhum projeto encontrado.</td></tr>`;
    qsa("[data-approve-proj]").forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { const ok = await confirmAction("Aprovar este projeto? Ele ficará visível na vitrine para todos.", { title: "Aprovar projeto", confirmLabel: "Sim, aprovar", neutral: true }); if (!ok) return; await adminFetch("/admin/moderate-project", { projectId: btn.getAttribute("data-approve-proj"), status: "published" }); toast("Projeto aprovado e na vitrine!", "success"); renderAll(); }))); qsa("[data-reject-proj]").forEach((btn) => btn.addEventListener("click", () => { qs("#rejectModal").style.display = "flex"; qs("#rejectProjectId", qs("#rejectModal")).value = btn.getAttribute("data-reject-proj"); })); qsa("[data-delproj]").forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { const ok = await confirmAction("Excluir este projeto permanentemente?", { title: "Excluir projeto" }); if (!ok) return; await adminFetch("/admin/delete-project", { projectId: btn.getAttribute("data-delproj") }); toast("Projeto excluído.", "success"); renderAll(); })));
  }

  function renderCategories() {
    qs("#catsTable tbody").innerHTML = db.categories.map((c) => `<tr><td>${escapeHtml(c.name)}</td><td>${db.projects.filter((p) => p.categoryId === c.id).length}</td><td><button class="btn btn-sm btn-danger" data-delcat="${c.id}">Remover</button></td></tr>`).join(""); qsa("[data-delcat]").forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { const id = btn.getAttribute("data-delcat"); if (db.projects.some((p) => p && p.categoryId === id)) { const ok = await confirmAction("Existem projetos usando essa categoria. Remover mesmo assim?", { title: "Remover categoria" }); if (!ok) return; } await adminFetch("/admin/delete-category", { categoryId: id }); toast("Categoria removida.", "success"); renderAll(); })));
  }

  function renderCommunity() {
    qs("#communityTable tbody").innerHTML = db.posts.map((p) => { const author = userById(p.authorId); return `<tr><td>${escapeHtml(author ? author.name : "—")}</td><td style="max-width:320px">${escapeHtml(p.content.slice(0, 140))}${p.content.length > 140 ? "…" : ""}</td><td>${p.comments ? p.comments.length : 0}</td><td>${fmtDate(p.createdAt)}</td><td><button class="btn btn-sm btn-danger" data-delpost="${p.id}">Excluir</button></td></tr>`; }).join("") || `<tr><td colspan="5" class="muted text-center">Nenhuma publicação na comunidade.</td></tr>`; qsa("[data-delpost]").forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { const ok = await confirmAction("Excluir esta publicação e todos os comentários?", { title: "Excluir publicação" }); if (!ok) return; await adminFetch("/admin/delete-post", { postId: btn.getAttribute("data-delpost") }); toast("Publicação removida.", "success"); renderAll(); })));
  }

  function renderReferrals() {
    qs("#refStatGrid").innerHTML = [ stat("Total de indicações", db.referrals.length), stat("Comissões geradas", fmtBRL(db.commissions.reduce((s, c) => s + c.amount, 0)), true), stat("Comissões pagas", fmtBRL(db.withdrawals.filter((w) => w.status === "approved").reduce((s, w) => s + w.amount, 0))) ].join(""); qs("#refsTable tbody").innerHTML = db.commissions.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((c) => { const label = { pending: ["badge-warning", "Pendente"], available: ["badge-success", "Disponível"], paid: ["badge-neutral", "Pago"] }[c.status]; return `<tr><td>${escapeHtml(userById(c.referrerId)?.name || "—")}</td><td>${escapeHtml(userById(c.referredId)?.name || "—")}</td><td>${fmtBRL(c.amount)}</td><td><span class="badge ${label[0]}">${label[1]}</span></td><td>${fmtDate(c.createdAt)}</td></tr>`; }).join("") || `<tr><td colspan="5" class="muted text-center">Nenhuma comissão registrada.</td></tr>`;
  }
  ...
  function renderRankingPrizes() {
    const table = qs("#rankingPrizesTable"); if (!table) return; const prizes = Object.values(db.rankingPrizes || {}).sort((a, b) => b.month.localeCompare(a.month) || a.category.localeCompare(b.category)); qs("tbody", table).innerHTML = prizes.map((p) => { const winner = p.winnerId ? userById(p.winnerId) : null; const catLabel = p.category === "indicacoes" ? "Mais indicações" : "Mais assinantes"; return `<tr><td>${escapeHtml(p.month)}</td><td>${catLabel}</td><td>${winner ? escapeHtml(winner.name) : "—"}${p.wasTiebreakDraw ? ' <span class="badge badge-neutral">sorteio</span>' : ""}</td><td>${p.score}</td><td>${p.delivered ? '<span class="badge badge-success">Entregue</span>' : '<span class="badge badge-warning">Pendente</span>'}</td><td>${ p.winnerId ? `<button class="btn btn-sm ${p.delivered ? "btn-ghost" : "btn-primary"}" data-toggle-delivered="${p.id}" data-current="${p.delivered ? "1" : "0"}">${p.delivered ? "Desmarcar" : "Marcar entregue"}</button>` : `<span class="muted">Sem vencedor</span>` }</td></tr>`; }).join("") || `<tr><td colspan="6" class="muted text-center">Nenhum mês fechado ainda.</td></tr>`; qsa("[data-toggle-delivered]", table).forEach((btn) => btn.addEventListener("click", () => withButtonLock(btn, async () => { await adminFetch("/admin/mark-prize-delivered", { prizeId: btn.getAttribute("data-toggle-delivered"), delivered: btn.getAttribute("data-current") !== "1" }); toast("Status atualizado.", "success"); renderAll(); })));
    let bwPanel = qs("#biweeklyPanel"); if (!bwPanel) { bwPanel = document.createElement("div"); bwPanel.id = "biweeklyPanel"; bwPanel.className = "panel"; bwPanel.style.marginTop = "24px"; bwPanel.innerHTML = `<div class="panel-head"><h3>Prêmios Quinzenais (Meta de Assinantes)</h3></div><div class="table-wrap"><table class="data-table" id="biweeklyPrizesTable"><thead><tr><th>Ciclo</th><th>Meta</th><th>Vencedor</th><th>Prêmio</th><th>Status</th><th>Ação</th></tr></thead><tbody></tbody></table></div>`; table.closest(".panel").parentNode.appendChild(bwPanel); }
    const bwTable = qs("#biweeklyPrizesTable tbody"); const bwPrizes = Object.values(db.biweeklyPrizes || {}).sort((a,b) => b.cycleStart.localeCompare(a.cycleStart)); let bwRows = ""; bwPrizes.forEach(cycle => { const cycleLabel = `${fmtDate(cycle.cycleStart)} a ${fmtDate(cycle.cycleEnd)}`; (cycle.winners || []).forEach(w => { const winnerUser = userById(w.winnerId); const isDelivered = cycle.delivered && cycle.delivered[w.threshold]; bwRows += `<tr><td>${cycleLabel}</td><td>${w.threshold} assinantes (Plano 4 dias)</td><td>${winnerUser ? escapeHtml(winnerUser.name) : "—"}</td><td>${fmtBRL(w.prize)}</td><td>${isDelivered ? '<span class="badge badge-success">Entregue</span>' : '<span class="badge badge-warning">Pendente</span>'}</td><td><button class="btn btn-sm ${isDelivered ? "btn-ghost" : "btn-primary"}" data-toggle-bw="${cycle.id}" data-threshold="${w.threshold}" data-current="${isDelivered ? "1" : "0"}">${isDelivered ? "Desmarcar" : "Marcar entregue"}</button></td></tr>`; }); }); bwTable.innerHTML = bwRows || `<tr><td colspan="6" class="muted text-center">Nenhum evento quinzenal fechado ainda.</td></tr>`; qsa("[data-toggle-bw]").forEach(btn => btn.addEventListener("click", () => withButtonLock(btn, async () => { await adminFetch("/admin/mark-biweekly-delivered", { cycleId: btn.getAttribute("data-toggle-bw"), threshold: btn.getAttribute("data-threshold"), delivered: btn.getAttribute("data-current") !== "1" }); toast("Prêmio quinzenal atualizado.", "success"); renderAll(); })));
    const closeBtn = qs("#forceCloseRankingBtn"); if (closeBtn && !closeBtn.dataset.bound) { closeBtn.dataset.bound = "1"; closeBtn.addEventListener("click", () => withButtonLock(closeBtn, async () => { const monthKey = qs("#forceCloseMonthInput")?.value.trim() || ""; const ok = await confirmAction(monthKey ? `Refazer ranking de ${monthKey} agora?` : "Fechar o ranking do mês anterior agora?", { title: "Fechamento manual", confirmLabel: "Sim, fechar agora" }); if (!ok) return; await adminFetch("/admin/force-close-ranking", monthKey ? { monthKey } : {}); toast("Ranking fechado.", "success"); renderAll(); })); }
  }

  function renderPlatformReviews() {
    const tbody = qs("#platformReviewsTable tbody"); if (!tbody) return; const reviews = db.platformReviews || []; if (reviews.length === 0) { tbody.innerHTML = `<tr><td colspan="6" class="text-center muted" style="padding: 24px;">Nenhuma avaliação da plataforma recebida ainda.</td></tr>`; return; }
    const sortedReviews = [...reviews].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)); const usaMap = { very_easy: "Muito fácil", easy: "Fácil", neutral: "Razoável", hard: "Difícil", very_hard: "Muito difícil" }; const recMap = { yes: "Sim", maybe: "Talvez", no: "Não" }; tbody.innerHTML = sortedReviews.map(r => { const date = fmtDate(r.createdAt); const starsHtml = `<span style="color: #facc15; font-size: 16px;">${"★".repeat(r.overallRating)}${"☆".repeat(5 - r.overallRating)}</span>`; return `<tr><td style="white-space: nowrap;">${date}</td><td><strong>${escapeHtml(r.userName)}</strong></td><td>${starsHtml}</td><td><span class="badge badge-neutral">${usaMap[r.usabilityScore] || r.usabilityScore}</span></td><td>${recMap[r.wouldRecommend] || r.wouldRecommend}</td><td style="max-width: 320px; white-space: normal; line-height: 1.4;">${escapeHtml(r.feedbackText || "—")}</td></tr>`; }).join("");
  }

  function bindForms() {
    const modal = qs("#rejectModal"); if (modal && !modal.dataset.bound) { modal.dataset.bound = "1"; qs("#rejectCancelBtn", modal).addEventListener("click", () => { modal.style.display = "none"; }); qs("#rejectForm", modal).addEventListener("submit", (e) => { e.preventDefault(); const submitBtn = modal.querySelector('button[type="submit"]'); withButtonLock(submitBtn, async () => { await adminFetch("/admin/moderate-project", { projectId: qs("#rejectProjectId", modal).value, status: "rejected", rejectReason: modal.querySelector('input[name="rejectReason"]:checked').value, }); toast("Projeto reprovado e usuário notificado.", "success"); modal.style.display = "none"; renderAll(); }); }); }
