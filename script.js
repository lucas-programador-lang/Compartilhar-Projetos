/* =========================================================
   COMPARTILHAR PROJETOS — SCRIPT.JS (v25 - Código 100% Completo)
   ========================================================= */
...
(function () {
  ...
  // TODAS AS PÁGINAS (HOME, RANKING, DASHBOARD, ETC) RESTAURADAS
  function monthRangeForBRT(date = new Date()) { const fmt = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Sao_Paulo", year: "numeric", month: "2-digit" }); const parts = fmt.formatToParts(date).reduce((acc, p) => { acc[p.type] = p.value; return acc; }, {}); const year = parseInt(parts.year, 10); const month = parseInt(parts.month, 10); const start = new Date(Date.UTC(year, month - 1, 1, 3, 0, 0)); const end = new Date(Date.UTC(month === 12 ? year + 1 : year, month === 12 ? 0 : month, 1, 3, 0, 0)); return { start, end }; }
  function currentMonthRanking() { const { start, end } = monthRangeForBRT(); const referralCounts = {}; db.referrals.forEach((r) => { if (r.createdAt) { const d = new Date(r.createdAt); if (d >= start && d < end) referralCounts[r.referrerId] = (referralCounts[r.referrerId] || 0) + 1; } }); const payingSets = {}; db.commissions.forEach((c) => { if (c.createdAt) { const d = new Date(c.createdAt); if (d >= start && d < end) { if (!payingSets[c.referrerId]) payingSets[c.referrerId] = new Set(); payingSets[c.referrerId].add(c.referredId); } } }); const toSortedList = (map) => Object.entries(map).map(([userId, count]) => ({ user: userById(userId), count })).filter((row) => row.user).sort((a, b) => b.count - a.count).slice(0, 20); return { byReferrals: toSortedList(referralCounts), byPaying: toSortedList(Object.fromEntries(Object.entries(payingSets).map(([id, set]) => [id, set.size]))) }; }
  function currentBiweeklyRanking() { const BIWEEKLY_EVENT_ANCHOR = "2026-08-25T03:00:00Z"; const anchor = new Date(BIWEEKLY_EVENT_ANCHOR); const now = new Date(); const daysSinceAnchor = Math.floor((now - anchor) / 86400000); const cycleIndex = Math.max(0, Math.floor(daysSinceAnchor / 15)); const cycleStart = new Date(anchor.getTime() + cycleIndex * 15 * 86400000); const cycleEnd = new Date(cycleStart.getTime() + 15 * 86400000); const commissions = db.commissions.filter(c => c && c.planId === "p4" && c.createdAt && new Date(c.createdAt) >= cycleStart && new Date(c.createdAt) < cycleEnd); const payingSets = {}; commissions.forEach((c) => { if (!payingSets[c.referrerId]) payingSets[c.referrerId] = new Set(); payingSets[c.referrerId].add(c.referredId); }); const counts = Object.entries(payingSets).map(([uid, set]) => ({ user: userById(uid), count: set.size })).filter(x => x.user).sort((a,b) => b.count - a.count); return { start: cycleStart, end: cycleEnd, ranking: counts }; }
  function rankingRow(row, i) { const pos = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i + 1}º`; return `<tr><td>${pos}</td><td><span class="pc-mini-avatar" style="background:${row.user.avatarColor || "#888"}">${initials(row.user.name)}</span> ${escapeHtml(row.user.name)}</td><td>${row.count}</td></tr>`; }
  ...
  function viewRanking() { const { byReferrals, byPaying } = currentMonthRanking(); const monthLabel = new Date().toLocaleDateString("pt-BR", { month: "long", year: "numeric" }); const prizes = pastPrizesList(); const biweekly = currentBiweeklyRanking(); const cycleLabel = `${fmtDate(biweekly.start)} até ${fmtDate(biweekly.end)}`; const content = `<section class="section" style="padding-top:44px"><div class="container"><span class="tag-label">Ranking de indicações</span><h2 style="margin-bottom:6px">Ranking de ${escapeHtml(monthLabel)}</h2><p class="field-hint" style="margin-bottom:26px">O ranking reinicia todo mês. No fechamento, o 1º lugar de cada categoria leva o prêmio (em caso de empate, sorteio automático decide).</p><div class="panel"><div class="panel-head"><h3>Mais indicações</h3></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Posição</th><th>Usuário</th><th>Indicações</th></tr></thead><tbody>${byReferrals.map(rankingRow).join("") || `<tr><td colspan="3" class="muted text-center">Ninguém indicou este mês ainda.</td></tr>`}</tbody></table></div></div><div class="panel"><div class="panel-head"><h3>Mais assinantes indicados</h3></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Posição</th><th>Usuário</th><th>Assinantes</th></tr></thead><tbody>${byPaying.map(rankingRow).join("") || `<tr><td colspan="3" class="muted text-center">Ninguém converteu assinante este mês ainda.</td></tr>`}</tbody></table></div></div><div class="panel"><div class="panel-head"><h3>Vencedores anteriores</h3></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Mês</th><th>Categoria</th><th>Vencedor</th><th>Pontuação</th><th>Prêmio</th></tr></thead><tbody>${prizes.map((p) => { const catLabel = p.category === "indicacoes" ? "Mais indicações" : "Mais assinantes"; return `<tr><td>${escapeHtml(p.month)}</td><td>${catLabel}</td><td>${p.winnerName ? escapeHtml(p.winnerName) : "—"}</td><td>${p.score}</td><td><span class="badge ${p.delivered ? "badge-success" : "badge-warning"}">${p.delivered ? "Entregue" : "Pendente"}</span></td></tr>`; }).join("") || `<tr><td colspan="5" class="muted text-center">Nenhum mês fechado ainda.</td></tr>`}</tbody></table></div></div><div class="panel mt-4" style="border-color: var(--gold-400)"><div class="panel-head"><div><h3 style="color: var(--gold-600)">Evento Quinzenal 🏆</h3><p class="muted" style="font-size:13px">Ciclo atual: ${cycleLabel}</p></div></div><p class="field-hint" style="margin: 0 16px 16px">Quem indicar mais assinantes do <strong>Plano 4 dias</strong> bate a meta primeiro e ganha prêmios de até R$ 50 direto no Pix. Duração de 15 dias!</p><div class="table-wrap"><table class="data-table"><thead><tr><th>Posição</th><th>Usuário</th><th>Planos de 4 Dias Ativos</th></tr></thead><tbody>${biweekly.ranking.map(rankingRow).join("") || `<tr><td colspan="3" class="muted text-center">Ninguém pontuou neste ciclo ainda.</td></tr>`}</tbody></table></div></div></div></section>`; const user = currentUser(); return user ? `<div class="dash-shell"><nav class="dash-sidebar">${sideNav("/ranking")}</nav><div class="dash-main">${content}</div></div>` : content; }
  function sideNav(active) { const items = [ ["/painel", "Visão geral"], ["/publicar", "Publicar"], ["/perfil", "Meu perfil"], ["/indicacoes", "Indicações"], ["/ranking", "Ranking"], ["/planos", "Assinatura"], ["/comunidade", "Comunidade"] ]; return `<div class="side-title">Meu painel</div>${items.map(([p, l]) => `<a class="side-link ${p === active ? "active" : ""}" href="#${p}">${l}</a>`).join("")}`; }
  function viewHome() { const publishedProjects = db.projects.filter((p) => p.status === "published"); const featured = publishedProjects.slice(0, 4); return `<section class="hero"><div class="container hero-inner"><div><span class="eyebrow"><span class="dot"></span> Plataforma de assinatura para criadores</span><h1>Encontre projetos, <span class="accent">compartilhe</span> suas ideias e conecte-se com criadores.</h1><p class="lead">Compartilhar Projetos é onde desenvolvedores, designers e criadores publicam o que estão construindo — e descobrem o que o resto da comunidade está criando.</p><div class="hero-cta"><a href="register.html" class="btn btn-gold btn-lg">Cadastrar grátis</a><a href="#/explorar" class="btn btn-ghost btn-lg btn-ghost-hero">Explorar projetos</a></div><div class="hero-stats"><div><strong>${publishedProjects.length}+</strong><span>projetos publicados</span></div><div><strong>${db.publicProfiles.length}+</strong><span>criadores cadastrados</span></div><div><strong>${db.categories.length}</strong><span>categorias ativas</span></div></div></div><div class="hero-visual" aria-hidden="true"><div class="fan-card fan-1"><span class="fc-cat">Design</span><h5>Verso Design System</h5><p>Componentes acessíveis para B2B.</p><div class="fc-foot"><span>Marina D.</span><span class="fc-badge">PRO</span></div></div><div class="fan-card fan-2"><span class="fc-cat">Mobile</span><h5>Trilha</h5><p>Trilhas offline para exploradores.</p><div class="fc-foot"><span>Marina D.</span><span>2 dias</span></div></div><div class="fan-card fan-3"><span class="fc-cat">Web</span><h5>Nimbus</h5><p>Financeiro para freelancers.</p><div class="fc-foot"><span>Marina D.</span><span class="fc-badge">PRO</span></div></div><div class="fan-card fan-4"><span class="fc-cat">IA</span><h5>Seu projeto aqui</h5><p>Publique e alcance a comunidade.</p><div class="fc-foot"><span>Você</span><span>Hoje</span></div></div></div></div></section><section class="section section-alt"><div class="container"><div class="section-head"><div><span class="tag-label">Como funciona</span><h2>Três passos para publicar seu projeto</h2></div></div><div class="steps-grid"><div class="step-card"><div class="step-num">Passo 1</div><h3>Crie sua conta grátis</h3><p>Cadastro leva menos de um minuto. Contas gratuitas já podem explorar todos os projetos publicados.</p></div><div class="step-card"><div class="step-num">Passo 2</div><h3>Assine um plano</h3><p>Escolha entre 4 ou 7 dias de publicação ativa. Pague uma vez e publique quantos projetos quiser no período.</p></div><div class="step-card"><div class="step-num">Passo 3</div><h3>Publique e conecte-se</h3><p>Adicione imagens, categoria, link e contato. Participe da comunidade e receba feedback de outros criadores.</p></div></div></div></section><section class="section"><div class="container"><div class="section-head"><div><span class="tag-label">Recém-publicados</span><h2>Projetos em destaque</h2></div><a href="#/explorar" class="link">Ver todos os projetos →</a></div><div class="project-grid">${featured.map(projectCard).join("") || emptyState("Nenhum projeto publicado ainda.")}</div></div></section><section class="section-tight"><div class="cta-band"><h2>Pronto para mostrar o que você está construindo?</h2><p>Assine um plano de publicação e coloque seu projeto na frente de toda a comunidade.</p><a href="#/planos" class="btn btn-gold btn-lg">Assinar para publicar</a></div></section>`; }
  ...
  function viewExplore(params) { const search = (params.q || "").toLowerCase(); let list = db.projects.filter((p) => p.status === "published"); if (search) list = list.filter((p) => p.title.toLowerCase().includes(search) || p.description.toLowerCase().includes(search)); const grouped = {}; db.categories.forEach(c => grouped[c.id] = { name: c.name, projects: [] }); grouped["geral"] = { name: "Geral", projects: [] }; list.forEach(p => { const cId = p.categoryId || "geral"; if(grouped[cId]) grouped[cId].projects.push(p); }); const categoriesHtml = Object.values(grouped).filter(g => g.projects.length > 0).map(g => `<div class="category-section" style="margin-bottom: 48px;"><h3 style="margin-bottom: 20px; border-bottom: 2px solid var(--gold-500, #d4af37); padding-bottom: 8px; display: inline-block;">${escapeHtml(g.name)}</h3><div class="project-grid">${g.projects.map(projectCard).join("")}</div></div>`).join(""); return `<section class="section" style="padding-top:44px"><div class="container"><div class="section-head"><div><span class="tag-label">Catálogo</span><h2>Explorar projetos</h2><p>Descubra o que criadores de todo o Brasil estão construindo agora.</p></div></div><div class="filters-bar" style="margin-bottom: 32px;"><input id="searchInput" class="search-input" type="search" placeholder="Buscar projetos por nome ou descrição…" value="${escapeHtml(params.q || "")}"></div>${categoriesHtml || emptyState("Nenhum projeto encontrado", "Tente ajustar a busca.")}</div></section>`; }
  function projectCard(p) { const img = p.images && p.images[0]; return `<a href="#/projeto/${p.id}" class="project-card"><div class="pc-thumb">${img ? `<img src="${img}" alt="${escapeHtml(p.title)}" loading="lazy">` : ""}<span class="pc-cat">${escapeHtml(categoryName(p.categoryId))}</span></div><div class="pc-body"><h3>${escapeHtml(p.title)}</h3><p>${escapeHtml(p.description)}</p><div class="pc-meta"><span class="author"><span class="pc-mini-avatar">${initials(p.ownerName)}</span>${escapeHtml(p.ownerName)}</span><span>${fmtDate(p.createdAt)}</span></div></div></a>`; }
  function viewProjectDetail(id) { const p = db.projects.find((pj) => pj.id === id); if (!p) return view404(); const img = (p.images && p.images[0]) || ""; const user = currentUser(); const projectReviews = p.reviews || []; const avgRating = projectReviews.length ? (projectReviews.reduce((acc, r) => acc + r.rating, 0) / projectReviews.length).toFixed(1) : 0; const hasRated = user ? projectReviews.some(r => r.userId === user.id) : false; let starsHtml = ""; for (let i = 1; i <= 5; i++) { starsHtml += getStarSvg(18, i <= Math.round(avgRating)); } let reviewsListHtml = projectReviews.map(r => { let rStars = ""; for (let i = 1; i <= 5; i++) { rStars += getStarSvg(14, i <= r.rating); } return `<div style="background: rgba(255,255,255,0.03); padding: 16px; border-radius: 12px; margin-bottom: 12px; border: 1px solid var(--border-color, rgba(255,255,255,0.05));"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;"><strong style="font-size: 14px; color: var(--text-main);">${escapeHtml(r.userName)}</strong><div style="display: flex; align-items: center; gap: 2px;">${rStars}</div></div><p style="font-size: 14.5px; color: var(--ink-700); margin: 0; line-height: 1.5;">${escapeHtml(r.comment)}</p><span style="font-size: 11px; color: var(--ink-300); display: block; margin-top: 8px;">${fmtDate(r.createdAt)}</span></div>`; }).join(""); if (projectReviews.length === 0) { reviewsListHtml = `<p class="muted" style="font-size: 14px; text-align: center; padding: 20px 0;">Ainda não há avaliações. Seja o primeiro a avaliar este projeto!</p>`; } const reviewLabel = projectReviews.length === 1 ? "avaliação" : "avaliações"; let reviewFormHtml = ""; if (!user) { reviewFormHtml = `<div class="panel text-center mt-3"><p class="muted">Faça login para avaliar.</p><a href="login.html?redirect=projeto/${p.id}" class="btn btn-ghost btn-sm mt-2">Entrar</a></div>`; } else if (hasRated) { reviewFormHtml = `<div class="panel text-center mt-3" style="border-color: var(--green-700);"><p style="color: var(--green-700); font-weight: 600; margin: 0;">✓ Você já avaliou este projeto.</p></div>`; } else if (user.id === p.ownerId) { reviewFormHtml = `<div class="panel text-center mt-3"><p class="muted" style="margin: 0;">Você não pode avaliar o seu próprio projeto.</p></div>`; } else { reviewFormHtml = `<form id="projectReviewForm" data-project="${p.id}" class="panel mt-3" style="background: var(--bg-card);"><h4 style="margin-bottom: 12px;">Deixe sua avaliação</h4><div style="margin-bottom: 16px;"><label class="muted" style="font-size: 12px; display: block; margin-bottom: 4px;">Nota (Estrelas)</label><div id="starRatingInput" style="display: flex; gap: 8px; cursor: pointer;"><span data-val="1">${getStarSvg(32, false)}</span><span data-val="2">${getStarSvg(32, false)}</span><span data-val="3">${getStarSvg(32, false)}</span><span data-val="4">${getStarSvg(32, false)}</span><span data-val="5">${getStarSvg(32, false)}</span></div><input type="hidden" name="rating" id="projectRatingVal" value="0" required></div><div class="field"><label>Comentário</label><textarea name="comment" rows="3" required placeholder="Conte à comunidade o que achou deste projeto..."></textarea></div><button type="submit" class="btn btn-primary btn-block">Enviar Avaliação</button></form>`; } return `<div class="project-detail container"><div class="breadcrumb"><a href="#/explorar">Explorar</a> / ${escapeHtml(categoryName(p.categoryId))} / <span class="muted">Detalhes</span></div><div style="margin-bottom: 24px; max-width: 760px;"><span class="pd-cat-badge">${escapeHtml(categoryName(p.categoryId))}</span><div style="margin-top: 14px;"><span class="muted" style="font-size: 11.5px; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 700; font-family: var(--font-mono);">Título</span><h1 class="pd-title" style="margin-top: 4px; margin-bottom: 8px;">${escapeHtml(p.title)}</h1><div style="display: flex; align-items: center; gap: 8px; margin-top: 8px;"><div style="display: flex;">${starsHtml}</div><span style="font-size: 14px; color: var(--ink-700); font-weight: 600;">${avgRating} <span style="font-weight: 400;">(${projectReviews.length} ${reviewLabel})</span></span></div></div></div><div class="pd-grid"><div class="pd-main"><div class="pd-gallery">${img ? `<img src="${img}" alt="${escapeHtml(p.title)}">` : `<span class="muted">Sem imagem</span>`}</div>${p.images && p.images.length > 1 ? `<div class="pd-thumbs">${p.images.map((im, i) => `<img src="${im}" class="${i === 0 ? "active" : ""}" alt="">`).join("")}</div>` : ""}<div style="margin-top: 32px;"><span class="muted" style="font-size: 11.5px; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 700; font-family: var(--font-mono);">Descrição</span><div class="pd-desc" style="margin-top: 8px; font-size: 15.5px; line-height: 1.7;">${escapeHtml(p.description)}</div></div><div style="margin-top: 48px; border-top: 1px solid var(--border-color, rgba(255,255,255,0.08)); padding-top: 32px;"><h3 style="margin-bottom: 20px; font-size: 20px;">Avaliações do Projeto</h3><div id="reviewsContainer">${reviewsListHtml}</div>${reviewFormHtml}</div></div><aside class="pd-side"><h4>Sobre o projeto</h4><div class="pd-row"><span>Responsável</span><span>${escapeHtml(p.ownerName)}</span></div><div class="pd-row"><span>Contato</span><span>${escapeHtml(p.contact)}</span></div><div class="pd-row"><span>Categoria</span><span>${escapeHtml(categoryName(p.categoryId))}</span></div><div class="pd-row"><span>Publicado em</span><span>${fmtDate(p.createdAt)}</span></div><a href="${escapeHtml(p.link)}" target="_blank" rel="noopener noreferrer" class="btn btn-primary btn-block" style="margin-top: 16px;">Acessar projeto ↗</a><button type="button" class="btn btn-outline-gold btn-block" style="margin-top:8px" onclick="compartilharConteudo('${escapeHtml(p.title)}', 'Olha esse projeto na plataforma:', location.origin + '/#/projeto/' + '${p.id}')">Compartilhar projeto</button></aside></div></div>`; }
  function viewPublish(params) { const user = currentUser(); if (!canPublish(user)) return `<div class="auth-shell"><div class="auth-card text-center"><h2>Assinatura necessária</h2><p class="sub">Para publicar projetos na plataforma, você precisa de uma assinatura ativa.</p><a href="#/planos" class="btn btn-gold btn-block">Ver planos de assinatura</a></div></div>`; let editProj = null; if (params && params.edit) editProj = db.projects.find(p => p.id === params.edit && p.ownerId === user.id); const isEdit = !!editProj; const titleVal = editProj ? escapeHtml(editProj.title) : ""; const descVal = editProj ? escapeHtml(editProj.description) : ""; const linkVal = editProj ? escapeHtml(editProj.link) : ""; const catIdVal = editProj ? editProj.categoryId : ""; const ownerNameVal = editProj ? escapeHtml(editProj.ownerName) : escapeHtml(user.name); const contactVal = editProj ? escapeHtml(editProj.contact) : escapeHtml(user.email); return `<section class="section" style="padding-top:44px;max-width:720px;margin:0 auto"><div class="container"><span class="tag-label">${isEdit ? "Revisão de Projeto" : "Novo projeto"}</span><h2 style="margin-bottom:6px">${isEdit ? "Editar e Reenviar Projeto" : "Publicar projeto"}</h2>${user.role === "admin" ? `<p class="field-hint" style="margin-bottom:20px">Você está publicando como administrador — não é necessário ter assinatura ativa.</p>` : `<div style="margin-bottom:26px"></div>`}${isEdit ? `<div class="badge badge-warning" style="margin-bottom: 20px; display: inline-block;">Corrija as informações abaixo para reenviar seu projeto.</div>` : `<p class="field-hint" style="margin-bottom:20px">Todas as postagens passam por revisão antes de serem publicadas na vitrine.</p>`}<form id="publishForm" class="panel">${isEdit ? `<input type="hidden" name="editingProjectId" value="${escapeHtml(editProj.id)}">` : ""}<div class="field"><label>Nome do projeto</label><input name="title" required placeholder="Ex.: Nimbus — painel financeiro" value="${titleVal}"></div><div class="field"><label>Descrição</label><textarea name="description" rows="5" required placeholder="Conte o que é, para quem serve e o que torna especial.">${descVal}</textarea></div><div class="field"><label>Categoria</label><select name="categoryId" required><option value="">Selecione…</option>${db.categories.map((c) => `<option value="${c.id}" ${c.id === catIdVal ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join("")}</select></div><div class="field"><label>Link do projeto</label><input name="link" type="url" required placeholder="https://" value="${linkVal}"></div><div class="field"><label>Nome do responsável</label><input name="ownerName" required value="${ownerNameVal}"></div><div class="field"><label>Forma de contato</label><input name="contact" required placeholder="E-mail ou telefone (WhatsApp)" value="${contactVal}"></div><div class="field"><label>Imagens do projeto ${isEdit ? "(Envie as imagens novamente)" : ""}</label><input type="file" id="imageInput" accept="image/*" multiple><div class="field-hint">Envie até 4 imagens do seu projeto.</div><div class="upload-preview" id="uploadPreview"></div></div><div class="field-error" id="publishError"></div><button class="btn btn-primary btn-block" type="submit">${isEdit ? "Reenviar Projeto" : "Publicar projeto"}</button></form></div></section>`; }
  function viewCommunity() { const user = currentUser(); const posts = db.posts.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)); return `<section class="section" style="padding-top:44px"><div class="container community-layout"><div><span class="tag-label">Comunidade</span><h2 style="margin-bottom:22px">Converse com outros criadores</h2>${user ? `<div class="composer"><textarea id="postInput" placeholder="Compartilhe uma novidade, peça feedback ou converse sobre a plataforma…"></textarea><div class="composer-foot"><span class="muted" style="font-size:12.5px">Publicando como ${escapeHtml(user.name)}</span><button class="btn btn-primary btn-sm" id="postSubmit">Publicar</button></div></div>` : `<div class="panel text-center"><p class="muted">Entre na sua conta para participar da comunidade.</p><a href="login.html?redirect=comunidade" class="btn btn-ghost btn-sm mt-2">Entrar</a></div>`}<div id="postsList">${posts.map((p) => { const author = userById(p.authorId); return `<article class="post-card" data-post="${p.id}"><div class="post-head"><div class="post-author"><span class="pc-mini-avatar" style="background:${author ? author.avatarColor : "#888"};width:34px;height:34px;font-size:13px">${initials(author ? author.name : "?")}</span><div><strong>${escapeHtml(author ? author.name : "Usuário removido")}</strong><br><span>${fmtDateTime(p.createdAt)}</span></div></div>${author && author.role === "admin" ? `<span class="badge badge-blue">Equipe</span>` : ""}</div><div class="post-body">${escapeHtml(p.content)}</div><div class="post-actions"><button class="comment-toggle" data-post="${p.id}">💬 ${p.comments.length} comentário(s)</button></div><div class="comment-section" data-post-comments="${p.id}" style="display:none"><div class="comment-list">${p.comments.map((c) => { const cAuth = userById(c.authorId); return `<div class="comment" data-comment="${c.id}"><span class="pc-mini-avatar" style="background:${cAuth ? cAuth.avatarColor : "#888"}">${initials(cAuth ? cAuth.name : "?")}</span><div style="flex:1"><div class="comment-body"><strong>${escapeHtml(cAuth ? cAuth.name : "Usuário removido")}</strong><p>${escapeHtml(c.content)}</p></div><div class="post-actions" style="border:none;padding-top:6px"><button class="reply-toggle" data-post="${p.id}" data-comment="${c.id}">Responder</button><span class="muted">${fmtDateTime(c.createdAt)}</span></div><div class="reply-list">${(c.replies || []).map((r) => { const rAuth = userById(r.authorId); return `<div class="comment"><span class="pc-mini-avatar" style="background:${rAuth ? rAuth.avatarColor : "#888"}">${initials(rAuth ? rAuth.name : "?")}</span><div class="comment-body"><strong>${escapeHtml(rAuth ? rAuth.name : "Usuário removido")}</strong><p>${escapeHtml(r.content)}</p></div></div>`; }).join("")}</div><form class="comment-reply-form" data-post="${p.id}" data-comment="${c.id}" style="display:none"><input type="text" placeholder="Escreva uma resposta…" required><button class="btn btn-sm btn-primary" type="submit">Enviar</button></form></div></div>`; }).join("")}</div><form class="comment-reply-form comment-new-form" data-post="${p.id}" style="margin-top:12px"><input type="text" placeholder="Escreva um comentário…" required><button class="btn btn-sm btn-primary" type="submit">Comentar</button></form></div></article>`; }).join("") || emptyState("Ainda não há publicações", "Seja a primeira pessoa a iniciar uma conversa.")}</div></div><aside><div class="side-card"><h4>Boas práticas</h4><p>Seja respeitoso, evite spam e mantenha o foco em projetos, feedback e aprendizado. Publicações ofensivas podem ser removidas pela moderação.</p></div><div class="side-card"><h4>Estatísticas</h4><p>${db.posts.length} publicações · ${db.publicProfiles.length} membros na comunidade.</p></div></aside></div></section>`; }
  function viewPlans() { const user = currentUser(); const active = isSubscriptionActive(user); return `<section class="section" style="padding-top:52px"><div class="container text-center" style="margin-bottom:44px"><span class="tag-label">Planos de assinatura</span><h2>Assine para publicar seus projetos</h2><p class="muted" style="max-width:520px;margin:12px auto 0">Contas gratuitas podem visualizar todos os projetos. Para publicar, escolha um plano abaixo.</p>${user && user.role === "admin" ? "" : active ? `<div class="badge badge-success mt-2">Assinatura ativa até ${fmtDate(user.subscription.expiresAt)}</div>` : user ? `<div class="badge badge-danger mt-2">Você ainda não tem assinatura ativa</div>` : ""}</div><div class="container"><div class="plans-grid"><div class="plan-card card-teste"><span class="plan-name">Plano Teste</span><div class="plan-price">R$ 5<span>,00</span></div><div class="plan-duration">Válido por 2 dias</div><ul class="plan-features"><li>Permite publicar projetos</li><li>Acesso à área de publicação</li><li>Expira automaticamente após 2 dias</li></ul><button class="btn btn-teste btn-block" data-plan="pTeste">Assinar plano teste</button></div><div class="plan-card card-4dias"><span class="plan-name">Plano 4 dias</span><div class="plan-price">R$ 10<span>,00</span></div><div class="plan-duration">Publicação ativa por 4 dias</div><ul class="plan-features"><li>Publique projetos ilimitados no período</li><li>Página individual para cada projeto</li><li>Participação na comunidade</li><li>Programa de indicação incluso</li></ul><button class="btn btn-4dias btn-block" data-plan="p4">Assinar plano 4 dias</button></div><div class="plan-card featured"><span class="plan-name">Plano 7 dias</span><div class="plan-price">R$ 20<span>,00</span></div><div class="plan-duration">Publicação ativa por 7 dias</div><ul class="plan-features"><li>Publique projetos ilimitados no período</li><li>Página individual para cada projeto</li><li>Participação na comunidade</li><li>Programa de indicação incluso</li><li>Mais tempo de visibilidade</li></ul><button class="btn btn-gold btn-block" data-plan="p7">Assinar plano 7 dias</button></div><div class="plan-card card-mensal"><span class="plan-name">Plano Mensal</span><div class="plan-price">R$ 50<span>,00</span></div><div class="plan-duration">Válido por 30 dias</div><ul class="plan-features"><li>Permite publicar projetos durante todo o período</li><li>Acesso completo à área de publicação</li><li>Expira automaticamente após 30 dias</li></ul><button class="btn btn-mensal btn-block" data-plan="pMensal">Assinar plano mensal</button></div></div></div></section>`; }
  ...
  function bindPageEvents(path) {
    ...
    if (copyBtn) { 
        copyBtn.addEventListener("click", () => { 
            ...
            if (isMobile && (typeof Android !== "undefined" || navigator.share)) { window.compartilharConteudo("Convite", "Cadastre-se:", linkText); } 
            else if (navigator.clipboard && window.isSecureContext) { navigator.clipboard.writeText(linkText).then(() => toast("Link copiado!", "success")); }
        }); 
    }
    ...
    if (projectReviewForm) {
      const stars = qsa("#starRatingInput span"); const ratingInput = qs("#projectRatingVal");
      stars.forEach(star => { star.addEventListener("click", () => { const val = parseInt(star.getAttribute("data-val")); ratingInput.value = val; stars.forEach(s => { const isFull = parseInt(s.getAttribute("data-val")) <= val; s.innerHTML = getStarSvg(32, isFull); }); }); });
      projectReviewForm.addEventListener("submit", (e) => {
        ...
        const commentInput = projectReviewForm.querySelector("textarea[name='comment']").value.trim(); const projectId = projectReviewForm.getAttribute("data-project"); const user = currentUser();
        ...
        const btn = projectReviewForm.querySelector("button[type='submit']"); btn.disabled = true; btn.textContent = "Enviando...";
    ...
    if (platformReviewForm) {
      const platformStars = qsa("#platformRatingStars span"); const platformRatingInput = qs("#overallRatingVal");
      platformStars.forEach(star => { star.addEventListener("click", () => { const val = parseInt(star.getAttribute("data-val")); platformRatingInput.value = val; platformStars.forEach(s => { const isFull = parseInt(s.getAttribute("data-val")) <= val; s.innerHTML = getStarSvg(32, isFull); }); }); });
      platformReviewForm.addEventListener("submit", async (e) => {
        ...
        try { await addPlatformReview(newPlatformReview); toast("Feedback enviado! Obrigado.", "success"); navigate("/"); } catch (err) { errorEl.textContent = err.message || "Ocorreu um erro."; errorEl.style.display = "block"; submitBtn.disabled = false; submitBtn.textContent = "Enviar Avaliação"; }
      });
    }
  }
