/* =========================================================
   COMPARTILHAR PROJETOS — SCRIPT.JS (v23 - Completo e Corrigido)
   ========================================================= */
...
(function () {
  ...
  function viewExplore(params) {
    const search = (params.q || "").toLowerCase(); let list = db.projects.filter((p) => p.status === "published");
  ...
  function viewProjectDetail(id) {
    ...
    const img = (p.images && p.images[0]) || ""; const user = currentUser();
    const projectReviews = p.reviews || []; const avgRating = projectReviews.length ? (projectReviews.reduce((acc, r) => acc + r.rating, 0) / projectReviews.length).toFixed(1) : 0;
    const hasRated = user ? projectReviews.some(r => r.userId === user.id) : false; let starsHtml = ""; for (let i = 1; i <= 5; i++) { starsHtml += getStarSvg(18, i <= Math.round(avgRating)); }
    let reviewsListHtml = projectReviews.map(r => { let rStars = ""; for (let i = 1; i <= 5; i++) { rStars += getStarSvg(14, i <= r.rating); } return `<div style="background: rgba(255,255,255,0.03); padding: 16px; border-radius: 12px; margin-bottom: 12px; border: 1px solid var(--border-color, rgba(255,255,255,0.05));"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;"><strong style="font-size: 14px; color: var(--text-main);">${escapeHtml(r.userName)}</strong><div style="display: flex; align-items: center; gap: 2px;">${rStars}</div></div><p style="font-size: 14.5px; color: var(--ink-700); margin: 0; line-height: 1.5;">${escapeHtml(r.comment)}</p><span style="font-size: 11px; color: var(--ink-300); display: block; margin-top: 8px;">${fmtDate(r.createdAt)}</span></div>`; }).join("");
    if (projectReviews.length === 0) { reviewsListHtml = `<p class="muted" style="font-size: 14px; text-align: center; padding: 20px 0;">Ainda não há avaliações. Seja o primeiro a avaliar este projeto!</p>`; }
    const reviewLabel = projectReviews.length === 1 ? "avaliação" : "avaliações"; let reviewFormHtml = "";
  ...
  function viewPublish(params) {
    ...
    let editProj = null; if (params && params.edit) editProj = db.projects.find(p => p.id === params.edit && p.ownerId === user.id); const isEdit = !!editProj;
  ...
  // EVENTOS DAS PÁGINAS (RESTUARADOS E CORRIGIDOS PARA EVITAR O ERRO)
