/* =========================================================
   COMPARTILHAR PROJETOS — SCRIPT.JS (v21 - A Digitar & Design)
   ========================================================= */
...
(function () {
  ...
  function initSupportChatWidget(user) {
      ...
      escutarPresencaAdmin((isOnline) => {
          if (isOnline) { statusText.innerHTML = `<span class="support-status-dot" style="background:#4ade80; box-shadow: 0 0 0 2px rgba(255,255,255,.35);"></span>Online agora`; }
          else { statusText.innerHTML = `<span class="support-status-dot" style="background:#94a3b8; box-shadow: 0 0 0 2px rgba(255,255,255,.35);"></span>Ausente`; }
      });

      function applyChatState(closed) {
          if (closed) { body.style.display = "none"; form.style.display = "none"; restartBox.style.display = "flex"; } 
          else { body.style.display = "flex"; form.style.display = "flex"; restartBox.style.display = "none"; }
      }
      applyChatState(isChatClosed);

      fab.addEventListener("click", () => {
          chatWin.classList.add("open"); badge.style.display = "none"; marcarChatLidoUser(user.id);
          if (!isChatClosed) setTimeout(() => input.focus(), 100); body.scrollTop = body.scrollHeight;
      });
      ...
      chatListenerUnsubscribe = escutarChatUsuario(user.id, (data) => {
          if (!data) return;
          if (data.status === "closed") { isChatClosed = true; localStorage.setItem(hiddenKey, "1"); applyChatState(true); return; } 
          else { isChatClosed = false; localStorage.removeItem(hiddenKey); applyChatState(false); }

          if (data.unreadUser && !chatWin.classList.contains("open")) { badge.style.display = "flex"; }

          // NOVO BANNER LIMPO E PROFISSIONAL
          ...
          if (!data.messages) {
              ...
              if (msgs.length > lastMsgCount && lastMsgCount > 0) { const last = msgs[msgs.length - 1]; if (last.sender === "admin") playSupportSound("receive"); }
              ...
              msgs.forEach(msg => {
                  ...
                  if (dateLabel !== lastDateLabel) {
                      const sep = document.createElement("div"); sep.style.cssText = "text-align: center; font-size: 10px; color: var(--ink-400); margin: 8px 0; font-weight: 700; text-transform: uppercase;"; sep.textContent = dateLabel; body.appendChild(sep); lastDateLabel = dateLabel;
                  }
                  const div = document.createElement("div"); div.className = `chat-msg ${msg.sender === "user" ? "user-sent" : "admin-received"}`;
  ...
  // --- FUNÇÕES DE ROTAS ORIGINAIS RESTAURADAS PARA EVITAR O ERRO 'NOT DEFINED' ---
  function viewHome() {
