...
(function () {
  ...
  function showPixModal({ pix }) {
    const overlay = document.createElement("div"); overlay.className = "modal-overlay open";
    // Correção: flex-shrink injetado diretamente em cada elemento interno do modal
    overlay.innerHTML = `<div class="modal-box" style="max-width:400px;text-align:center;display:flex;flex-direction:column;"><button type="button" class="modal-close" id="pixCloseBtn" aria-label="Fechar">×</button><h2 style="flex-shrink:0">Pague com Pix para ativar sua assinatura</h2><div id="pixQrCode" class="pix-qr" style="width:220px;height:220px;min-width:220px;min-height:220px;flex-shrink:0;margin:16px auto;display:flex;align-items:center;justify-content:center"><span class="muted" style="font-size:12px">Gerando QR Code…</span></div><textarea readonly style="width:100%;flex-shrink:0;font-size:11px;padding:8px" rows="4">${pix.code || ""}</textarea><button id="pixCopyBtn" class="btn btn-primary btn-sm mt-2" style="flex-shrink:0">Copiar código</button><p class="muted mt-2" style="font-size:13px;flex-shrink:0">Assim que o pagamento for confirmado, sua assinatura ativa automaticamente — não precisa recarregar a página.</p></div>`;
